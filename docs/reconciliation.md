# Сверка: PostgreSQL и ручная выгрузка

Сверка данных ETL с ручной выгрузкой из TG Booster.

## Период (пример)

| Поле | Значение |
|------|----------|
| Дата начала | 2026-07-07 |
| Дата окончания | 2026-07-13 |
| Источник (ручной) | Excel/CSV, уровень объявлений |
| Источник (БД) | `raw_campaign_stats` |

## Итог за период

| Метрика | Ручная выгрузка | PostgreSQL | Расхождение |
|---------|-----------------|------------|-------------|
| Расход | 13 325.21 | 13 328.53 | **+0.025%** |
| Лиды | 2 810 | 2 811 | **+0.036%** |

**Вывод:** погрешность < 2% — данные ETL совпадают с ручной выгрузкой.

## Почему не 0%

Цифры редко сходятся на 100%:

- **Ручная выгрузка** — детальнее, уровень объявлений
- **База** — агрегат по кампаниям (день × кампания × кабинет)
- **Округление** при суммировании

Расхождение **0,025–0,036%** за неделю — для weekly-отчёта это совпадение, а не ошибка пайплайна.

## Сверка по дням

| Дата | Расход (ручной) | Расход (БД) | Расхождение |
|------|-----------------|-------------|-------------|
| 2026-07-07 | 2 118.38 | 2 118.98 | +0.03% |
| 2026-07-08 | 2 048.35 | 2 051.38 | +0.15% |
| 2026-07-09 | 2 046.79 | 2 047.14 | +0.02% |
| 2026-07-10 | 1 964.44 | 1 964.26 | −0.01% |
| 2026-07-11 | 1 721.06 | 1 720.99 | −0.00% |
| 2026-07-12 | 1 854.08 | 1 853.97 | −0.01% |
| 2026-07-13 | 1 572.11 | 1 571.81 | −0.02% |

## Как повторить

```bash
# Итоги в БД за период
python -m etl reconcile --from 2026-07-07 --to 2026-07-13

# Сравнение с Excel-выгрузкой
python scripts/compare_manual_export.py path/to/export.xlsx <account_id>
```

## SQL

```sql
SELECT
    stat_date,
    SUM(spend) AS spend,
    SUM(leads) AS leads
FROM raw_campaign_stats
WHERE account_id = '<cabinet_id>'
  AND stat_date BETWEEN '2026-07-07' AND '2026-07-13'
GROUP BY stat_date
ORDER BY stat_date;
```
